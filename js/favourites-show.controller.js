(function () {
    ...
    /* @ngInject */
    function FavouritesShowController($location, ENV, $scope, $http, $rootScope, $stateParams, selectedMovieService, $state,
                                      $ionicLoading, $ionicPopup, $ionicModal, cinemasList, $ionicHistory, craigalytics) {
        /* jshint validthis: true */
        var vm = this;

        vm.activate = activate;
        vm.title = 'FavouritesShow';
        vm.changeCinema = changeCinema;
        ...
        function activate() {
            setCinema($rootScope.favouriteCinema);
        }

        function setCinema(cinema) {
            vm.favouriteCinema = cinema;
            $rootScope.favouriteCinema = cinema;
            if(!cinema.id) {
                $scope.groups = [];
                // empty cinema
                return;
            ...
            loadMovies(false, time);
        }
        function changeCinema(cinema) {
            setCinema(cinema);
        }
        ...
        function loadMovies(force, startingAfterTime) {
            var cinemaObj = vm.favouriteCinema;
